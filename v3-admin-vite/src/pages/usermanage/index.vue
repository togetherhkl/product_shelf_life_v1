...
function getAvatarUrl(avatarUrl?: string) {
  ...
  if (!avatarUrl) return ""
  // 如果不是以http开头的完整URL，则拼接服务器地址
  if (!avatarUrl.startsWith("http")) {
    console.log(`${baseURL}${avatarUrl}`) // 完整头像地址
    return `${baseURL}${avatarUrl}`
  }
  // 如果是完整URL（http或https），直接返回
  return avatarUrl
}
          ...
          <el-table-column prop="avatar_url" label="头像" align="center" width="80">
            <template #default="scope">
              <el-avatar
                ...
                :src="getAvatarUrl(scope.row.avatar_url)"
                size="small"
                @error="() => {}"
              >
                <template #default>
    ...
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="left">
        ...
        <el-form-item prop="avatar_url" label="头像URL">
          ...
          <!-- 头像预览 -->
          <div v-if="formData.avatar_url" class="avatar-preview" style="margin-top: 10px;">
            <el-text>预览：</el-text>
            <el-avatar
